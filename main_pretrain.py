...
def get_args_parser():
    ...
    parser.add_argument('--pretrained_weight', type=str, default="./mae-base-checkpoint-799.pth")
...
def main(args):
    ...
    dataset_train = util.datasets.HDF5ImageNetDataset(os.path.join(args.data_path, 'ImageNet.hdf5'), "train", transform=transform_train)
    ...
    collate_fn = lambda b: util.datasets.fast_collate(b, torch.contiguous_format)
    ...
    scorer.load_state_dict(torch.load(args.pretrained_weight, map_location="cpu")["model"])
    ...
    misc.load_model(args=args, model_without_ddp=model_without_ddp, optimizer=optimizer, loss_scaler=loss_scaler, optimizer_d=optimizer_d, discriminator_without_ddp=discriminator_without_ddp)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.output_dir and (epoch % 5 == 0 or (epoch + 1) % 100 == 0 or epoch + 1 == args.epochs):
            misc.save_model(
                args=args, model=model, model_without_ddp=model_without_ddp, optimizer=optimizer,
                loss_scaler=loss_scaler, epoch=epoch, optimizer_d=optimizer_d, discriminator=discriminator_without_ddp)
